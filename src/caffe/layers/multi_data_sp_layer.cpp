...
#include "caffe/layers/multi_data_sp_layer.hpp"
...
MultiDataSpLayer<Dtype>::~MultiDataSpLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void MultiDataSpLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  string sp_folder = this->layer_param_.multi_data_param().root_folder()+"../superpixel/";
  ...
  cv::Mat biasImg =  ReadImageToCVMat(sp_folder + lines_[lines_id_].second,
...
template <typename Dtype>
void MultiDataSpLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void MultiDataSpLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
...
INSTANTIATE_CLASS(MultiDataSpLayer);
REGISTER_LAYER_CLASS(MultiDataSp);
